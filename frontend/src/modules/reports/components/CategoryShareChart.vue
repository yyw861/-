...
function label(item:CategoryShare){return item.subCategoryName ? `${item.subCategoryCode} ${item.subCategoryName}` : `${item.categoryCode} ${item.categoryName}`}
function render(){chart?.setOption({tooltip:{trigger:'item',valueFormatter:(value:unknown)=>formatCurrency(Number(value))},legend:{bottom:0},series:[{name:'分类销售占比',type:'pie',radius:['38%','68%'],data:props.items.map(item=>({name:label(item),value:Number(item.netSalesAmount)}))}]},true)}
...
<template><div ref="root" class="chart" role="img" aria-label="分类销售占比图"></div><ul class="sr-only"><li v-for="item in items" :key="item.subCategoryId ?? item.categoryId">{{label(item)}}：{{formatCurrency(item.netSalesAmount)}}</li></ul></template>
